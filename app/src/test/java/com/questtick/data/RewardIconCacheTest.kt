...
class RewardIconCacheTest {
    @Test
    fun concurrentSameUrlDownloadsOnceAndLaterReusesLocalFile() =
        runBlocking {
    ...
    @Test
    fun prefersOriginalImageWhenCdnUrlContainsResizeParameter() =
        runBlocking {
                ...
                val transport =
                    FakeHttpTransport { request ->
                        if (request.url == original) {
                            HttpResponse.bytes(200, byteArrayOf(7, 8, 9), request.url)
                        } else {
