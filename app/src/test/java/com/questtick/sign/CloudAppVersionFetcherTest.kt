...
class CloudAppVersionFetcherTest {
    @Test
    fun `cloud genshin reads version from its App Store lookup`() =
        runBlocking {
            var requestedUrl = ""
            val transport =
                HttpTransport { request: HttpRequest ->
    ...
    @Test
    fun `cloud star rail reads version from its App Store lookup`() =
        runBlocking {
            var requestedUrl = ""
            val transport =
                HttpTransport { request: HttpRequest ->
    ...
    @Test
    fun `invalid App Store response is reported as failure`() =
        runBlocking {
