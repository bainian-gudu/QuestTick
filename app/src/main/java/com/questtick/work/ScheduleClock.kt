...
internal fun selectScheduleTime(sources: ScheduleTimeSources): Long {
    ...
    val agreeingPairs = candidates.flatMapIndexed { index, first ->
    ...
    return when {
        agreeingPairs.isNotEmpty() -> agreeingPairs.maxBy { pair ->
...
class ScheduleClock @Inject constructor(
    ...
    private data class NetworkSample(val wallMillis: Long, val elapsedMillis: Long)
    ...
    private suspend fun fetchNetworkTime(receivedElapsed: Long): Long? = runCatching {
        val response = httpTransport.execute(
            HttpRequest(
