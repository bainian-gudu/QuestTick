...
object AppUpdateNetwork {
    ...
    fun networkLevel(context: Context): NetLevel = try {
    ...
    fun adaptiveTimeout(context: Context, baseConnectMs: Long, baseReadMs: Long): Pair<Long, Long> =
    ...
    fun buildCandidateUrls(originalUrl: String): List<String> =
        originalUrl.takeIf(TrustedUrlPolicy::isUpdateSourceUrl)?.let { listOf(it) }.orEmpty()
